...
subroutine mpc(ue, a, jdof, mdof, n, jtype, x, u, uinit, maxdof,
  ...
  1                         beta(quatdim, quatdim)
   ...
   integer               ::  coord_field
   parameter                 (coord_field=1)
   real(8)               ::  b_mat_current(quatdim, quatdim)
   real(8), allocatable  ::  c_mat_current(:, :)
   real(8)               ::  quat_increment(quatdim), 
  1                          quat_current(quatdim)
   real(8)               ::  cent_current(ndim)
   ...
   allocate(du_shell(ndim, n_shell))
   ...
   allocate(c_mat_current(ndim, n_shell))
   allocate(q_mat_current(ndim, ndim * n_shell))
C ******************************************************************** C
C Subroutine definition
C ******************************************************************** C
   ! Calculate the centroid locations
   !do while (my_var .NE. 999)
   !  my_var = 1
   !end do
  ...
  1                         r_mat)
   ...
   ! Calculate the increment in displacement
   cent_current(:) = zero
   do i = 1, n_shell
     cent_current = cent_current + field(1:3, coord_field, i + 1)
   end do
   cent_current = cent_current * n_reciprocal
   do i = 1, n_shell
     c_mat_current(1:3, i) = field(1:3, 1, i + 1) - cent_current
   end do
   
   ! Calculate the current rotation
   b_mat_current(:, :) = zero
   do i = 1, n_shell
     beta = left_quat(zero, c_mat_current(:, i))
  1         - right_quat(zero, c_mat(:, i))
     b_mat_current = b_mat_current + weights(1, i) / total_weight 
  1                  * matmul(transpose(beta), beta)
   end do
   lam_q = calc_opquat(b_mat_current)
   lambda = lam_q(1)
   quat_current = lam_q(2:5)
   ! Make the first entry always positive
   quat_current = sign(one, quat_current(1)) * quat_current
   
   ! Calculate the rotation increment
   b_mat_current(:, :) = zero
   do i = 1, n_shell
     beta = left_quat(zero, d_mat(:, i))
  1         - right_quat(zero, c_mat_current(:, i))
     b_mat_current = b_mat_current + weights(1, i) / total_weight 
  1                  * matmul(transpose(beta), beta)
   end do
   lam_q = calc_opquat(b_mat_current)
   lambda = lam_q(1)
   quat_increment = lam_q(2:5)
   ! Make the first entry always positive
   quat_increment = sign(one, quat_increment(1)) * quat_increment
   
   ! Get the total rotation
   quat_current = quat_prod(quat_increment, quat_current)
         
   ! Linearization of the increment
   r_mat = rot_mat(quat_current)
   g_mat = calc_g(quat_current, n_shell,b_mat_current,c_mat_current,
  1               lambda, r_mat)
   q_mat_current = calc_q(quat_current, n_shell, g_mat)
   ...
   do i = 2, n
     ...
     w_ish = weights(1, i_sh) / total_weight
     ! Rotation constraints
     !a(4, 1:3, i) = -q_mat_current(1, 3*i_sh-2:3*i_sh) * w_ish
     !a(5, 1:3, i) = -q_mat_current(2, 3*i_sh-2:3*i_sh) * w_ish
     !a(6, 1:3, i) = -q_mat_current(3, 3*i_sh-2:3*i_sh) * w_ish
   end do
   
   ! Printing
   if (abs(x(3, 1) - 5250.) < 0.1) then
     print *, 'z = ', x(3, 1), 'inc = ', kinc
     print *, 'rot diff', norm2(extract_rotation(op_quat)
  1           - extract_rotation(quat_current))
  2           / norm2(extract_rotation(op_quat))
     print *, 'norm q_mat', mat_norm(q_mat, ndim, ndim * n_shell)
     print *, 'norm q_mat_current', mat_norm(q_mat_current, ndim, 
  1            ndim * n_shell)
     !print *, 'current rot?'
     !print *, u(4:6, 1)
     !print *, uinit(4:6, 1)
     ...
     !print *, 'cent diff'
     !print *, u(1:3, 1) + (xc - cent_current) - xc
     !print *, 'c mat current'
     !print *, c_mat_current
     !print *, 'd mat'
     !print *, d_mat
     print *, 'rot lin total'
     do i = 1, n_shell
       print *, -q_mat(1, 3*i-2:3*i) * w_ish
     end do
     print *, 'rot lin increment'
     do i = 1, n_shell
       print *, -q_mat_current(1, 3*i-2:3*i) * w_ish
     end do
     print *, 'x current'
     do i = 1, n_shell
       print *, field(1:3, coord_field, i + 1)
     end do
   end if
   ...
   ue(1:3) = u_cent
   ue(4:6) = extract_rotation(quat_current)
   ue(7) = w_amp
   return
   
   contains
C ******************************************************************** C
C     Matrix norm wrapper
C ******************************************************************** C
   ! Returns the matrix norm
   ! @ input aa: Matrix to calculate the norm
   ! @ input rows: Number of rows
   ! @ input cols: Number of columns
   ! @ returns val: Matrix norm
   function mat_norm(aa, rows, cols) result(val)
   integer, intent(in) ::  rows, cols
   real(8), intent(in) :: aa(rows, cols)
   real(8)             :: val
   ! Internal variables
   real(8)             ::  work(rows)
   external dlange
   ! 
   val = dlange('f', rows, cols, aa, rows, work)
   end function
C ******************************************************************** C
...
C ******************************************************************** C
   ! Returns the product of q1 \circ q2
   ! @ input q1: Left quaternion, length 4
   ! @ input q2: Right quaternion, length 4
   ! @ return q3: Quaternion product
   pure function quat_prod(q1, q2) result(q3)
   real(8), intent(in) :: q1(4), q2(4)
   real(8)             :: q3(4)
   ! 
   q3(1) = q1(1) * q2(1) - dot_product(q1(2:4), q2(2:4))
   q3(2) = q1(1) * q2(2) + q2(1) * q1(2) + q1(3)*q2(4) - q1(4)*q2(3)
   q3(3) = q1(1) * q2(3) + q2(1) * q1(3) - q1(2)*q2(4) + q1(4)*q2(2)
...
C ******************************************************************** C
C ******************************************************************** C
C     Put previous coordinates as a field variable
C ******************************************************************** C
C ******************************************************************** C
