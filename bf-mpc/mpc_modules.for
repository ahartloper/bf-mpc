    ...
    ! TODO: 
    ! - get away from using the normalized definitions of everything
    ! - clean up the code

      include "jacobi_eigenvalue.for"
    ...
    ! ******************************************************************** !
     ...
     1      * spread(init_pts(:, ii), dim=1, ncopies=3)
        ...
        ! The eigenvalues are returned in descending order (smallest first)
        o = order_ini(v)
      end function
    ...
    ! ******************************************************************** !
      ...
      pure function warp_amp2(rel_pos_def, n_sh, t, psi) result(w)
      ! Returns the value of the averaged warping amplitude.
      ! @ input x_ini: pos. of each node in the initial configuration
      ! @ input x_def: pos. of each node in the deformed configuration
      ! @ input u_c: Translation of the centroid
      ...
      ! @ returns w: Warping amplitude
        ! Input and output
        implicit none
        real(8), intent(in) ::  rel_pos_def(:, :), t(3)
        ...
        w_denom = 0.d0
        do ii = 1, n_sh
          w = w + psi(ii) * dot_product(t, rel_pos_def(1:3, ii))
          w_denom = w_denom + psi(ii) ** 2
        end do
        w = w / w_denom
      end function
    ...
    ! ******************************************************************** !
      ...
      pure function shear_factors(p, n_sh, sec_props, mesh_sizes,c_tags)
     1       result(v)
    ...
    ! ******************************************************************** !
      ...
      pure function section_tangent(n_sh, xy_bar, da, etang, 
     1                               section_props) result(ksec)
    ...
    ! ******************************************************************** !
      ...
      pure function nodal_forces(n_sh, xy_bar, da, etang, usec) 
     1       result(f_nodes)
    ...
    ! ******************************************************************** !
     ...
     1     classification, mesh_sizes) result(areas)
        ...
        real(8), intent(in) ::  xyz(3, n_sh), sec_props(4), 
     1                           mesh_sizes(2)
        real(8)             ::  areas(n_sh)
        integer             ::  i
        ...
        integer             ::  web_tag, flange_tag, corner_tag,
     1                           joint_tag
        parameter               (web_tag=1, flange_tag=2, 
     1                            corner_tag=3, joint_tag=4)
        parameter(tol=1.d-3)
        ! Function start
    ...
    ! ******************************************************************** !
      ...
      pure function compute_mesh_size(n_sh,p,sec_props,classification)
     1         result(mesh_sizes)
        implicit none
