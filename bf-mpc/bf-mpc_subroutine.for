...
! Written by: A Hartloper, EPFL, alexander.hartloper@epfl.ch
      ...
      include 'mpc_modules.for'
      include 'uexternaldb_subr.for'

      subroutine mpc(ue, a, jdof, mdof, n, jtype, x, u, uinit, maxdof, 
     1               lmpc, kstep, kinc, time, 
     2               nt, nf, temp, field, ltran, tran)
      use mpc_modules
     ...
     1                         rot_lin(:, :), x_shell_def(:, :), 
     2                         x_shell(:, :), u_shell(:, :), 
     3                         warp_lin(:), psi_all(:), weights(:, :), 
     4                         x_shell_ref(:, :), node_areas(:), 
     5                         shear_weights(:, :)
     ...
     2                         wtmod(:), tmod(:), tmod0(:), 
     3                         xy_bar(:, :), f_nodes(:, :)
      real(8)               ::  temp33(3, 3), wtmod_tot, r1, 
     1                          ksec(4, 4), usec(4, 4)
      ...
      n_shell = n - 1
      ! todo: need to change all the allocates to fixed size...
      ...
      mesh_sizes = compute_mesh_size(n_shell, x_shell_ref, 
     1                               section_props, node_classes)
      node_areas = compute_node_areas(n_shell, x_shell_ref,
     1                         section_props, node_classes, mesh_sizes)
      xy_bar = normalized_coords(n_shell, x_shell_ref(1:2, :), 
     1                           section_props(1), section_props(2))
      area_and_moi = compute_section_props(n_shell,x_shell_ref(1:2, :),
     1                                     node_areas)
      ...
      shear_weights = shear_factors(x_shell_ref, n_shell, 
     1                      section_props, mesh_sizes, node_classes)
      ! Assume elastic, ksec is identity, else can compute the section stiffness
      forall(i = 1:4) ksec(i, i) = one
      usec = calc_section_deform(ksec, area_and_moi, section_props(1),
     1                           section_props(2))
